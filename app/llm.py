...
_INVALID_REPLY_PATTERNS = (
    r"^(okay|alright|let me|we need to|i need to|first,? i|the user is|the task is)\b",
    r"\bkey points from (the )?instructions\b",
    r"\bmy task is to\b",
    r"\bthe employer'?s (message|reply|vacancy)\b",
    r"^(analysis|reasoning|thought process)\s*:",
    r"^(давайте|сначала)\s+(разбер|проанализ)",
    r"^__skip__$",
...
def generate_llm_reply(conversation: list, employer_name: str = "", cover_letter: str = "", resume_text: str = "", account_key: str = "", ai_screener_hint: bool = False) -> str:
    ...
    system += (
        ...
        "\n\nВерни только готовый текст сообщения соискателя, без анализа, рассуждений, "
        "Markdown и пояснений. Не придумывай конкретные технологии, проекты, компании, "
        "должности, сроки опыта, город или готовность к переезду, если этого нет в резюме. "
        "Допустимо лишь увереннее сформулировать реальные смежные навыки и способность быстро адаптироваться. "
        "Если прямого опыта нет — скажи об этом честно и кратко."
    )
    ...
    if mode == "roundrobin":
        ...
        try:
            ...
            if _looks_like_invalid_reply(result) or not _looks_like_direct_answer(conversation, result):

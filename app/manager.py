...
def _employer_message_requires_reply(text: str) -> bool:
    """Conservative guard: draft only for a real question or actionable request."""
    ...
    no_reply_patterns = (
        ...
        "однако, в данный момент мы не готовы",
        "вернемся к вашей кандидатуре",
        "вернёмся к вашей кандидатуре",
        "рассмотрим ваше резюме",
        "если ваш опыт и навыки подойдут",
        "если ваши опыт и навыки подойдут",
        "сохраним ваше резюме",
        "желаем удачи в поиске",
        "не забудьте пройти первичное интервью",
        "вот ссылка на анкету",
        "ссылка на анкету-знакомство",
        "по результатам анкеты",
    )
    if any(pattern in normalized for pattern in no_reply_patterns):
        return False
    request_markers = (
        "?", "расскажите", "подскажите", "уточните", "ответьте",
        "готовы ли", "есть ли", "был ли", "были ли", "сколько",
        "когда", "можете ли", "сможете ли", "ожидания", "созвон",
...
class BotManager:
    ...
    @staticmethod
    def _llm_hh_identity(state: AccountState) -> str:
        """Stable HH-user identity shared by cloned resume sessions."""
        cookies = state.acc.get("cookies") or {}
        for key in ("crypted_hhuid", "hhuid", "hhtoken"):
            value = str(cookies.get(key) or "").strip()
            if value:
                return f"{key}:{value}"
        # Sessions without identity cookies must never be grouped accidentally.
        return f"state:{id(state)}"

    def _llm_profile_owner(self, state: AccountState, vacancy_id: str) -> tuple[bool, str]:
        """Choose exactly one resume session to handle a shared-account HH chat.

        Prefer the profile recorded for the original application. If the vacancy
        is unknown locally (for example, a manual application), choose the first
        active profile deterministically so parallel workers cannot both call LLM.
        """
        identity = self._llm_hh_identity(state)
        all_states = list(self.account_states) + list(self.temp_states.values())
        siblings = [
            candidate for candidate in all_states
            if not getattr(candidate, "_deleted", False)
            and self._llm_hh_identity(candidate) == identity
        ]
        if state not in siblings:
            siblings.append(state)

        applied_accounts = get_applied_accounts(vacancy_id)
        matched = [
            candidate for candidate in siblings
            if candidate.name in applied_accounts or candidate.short in applied_accounts
        ]
        pool = matched or [
            candidate for candidate in siblings
            if getattr(candidate, "llm_enabled", True) and not getattr(candidate, "paused", False)
        ] or siblings
        owner = pool[0]
        return owner is state, owner.short
    ...
    def _run_account_worker_inner(self, idx: int, state: AccountState) -> None:
        ...
        # После рестарта in-memory daily_sent раньше начинался с нуля, а
        # 30-секундный limit tracker успевал синхронизироваться только после
        # старта отправки. В результате worker мог превысить пользовательский
        # дневной потолок. До первого поиска всегда сверяемся с HH.
        try:
            daily_info = fetch_negotiations_today_count(acc)
            if daily_info:
                real_today = int(daily_info.get("today", 0) or 0)
                configured_limit = CONFIG.daily_apply_limit or CONFIG.hh_daily_limit or 200
                with state._state_lock:
                    state.daily_sent = real_today
                    state.hh_today_applies = real_today
                    state.hh_today_applies_updated = datetime.now().isoformat(timespec="seconds")
                    if configured_limit > 0 and real_today >= configured_limit:
                        ...
                        state.status = "limit"
                        state.status_detail = (
                            f"Дневной лимит: {real_today}/{configured_limit}. "
                ...
                if configured_limit > 0 and real_today >= configured_limit:
                    self._add_log(
                        state.short,
                        state.color,
                        f"🛑 До старта синхронизирован HH-лимит: "
                        f"{real_today}/{configured_limit}. Отклики не запущены",
                        "warning",
                    )
        except Exception as e:
            # Сетевой сбой синхронизации не должен ронять worker, но остаётся
            # видимым в диагностике.
            log_debug(f"daily preflight sync [{state.short}] failed: {e}")
        ...
        while not self._stop_event.is_set() and not state._deleted:
            ...
            # rating_skipped исторически включает не только рейтинг, но и
            # quality gates (auto_response / IT accreditation). Старый текст
            # «рейтинг» вводил в заблуждение: в UI казалось, что вакансии
            # отброшены только из-за оценки работодателя.
            quality_msg = f", 🛡️ качество {state.rating_skipped}" if state.rating_skipped else ""
            self._add_log(
                state.short, state.color,
                f"\U0001f50d Фильтрация: ✅ уже {already_count}, \U0001f9ea тест {test_count}{sal_msg}{sched_msg}{title_msg}{discard_msg}{quality_msg}, \U0001f195 новые {len(filtered)}",
                "info",
            )

            if not filtered:
                state.status = "waiting"
                state.status_detail = "Нет вакансий после фильтров"
                self._add_log(
                    state.short, state.color,
                    "⚠️ После фильтров не осталось вакансий: "
                    f"заголовок {title_skipped}, отказы {discard_skipped}, "
                    f"качество {state.rating_skipped}, уже {already_count}, "
                    f"тесты {test_count}. Повтор через 2 мин",
                    "warning",
                )
                time.sleep(120)
                continue
        ...
        while not self._stop_event.is_set():
            try:
                ...
                for state in states:
                    ...
                    # Всегда синхронизируем in-memory счётчик с HH. Раньше это
                    # происходило только при recovery и после рестарта UI мог
                    # показывать 0, пока worker уже отправлял новые отклики.
                    limit = CONFIG.daily_apply_limit or CONFIG.hh_daily_limit or 200
                    with state._state_lock:
                        state.daily_sent = count

                    if limit > 0 and count >= limit:
                        was_limited = state.hard_stopped and state.paused_reason == "limit"
                        with state._state_lock:
                            state.hard_stopped = True
                            state.limit_exceeded = True
                            if state.paused_reason != "manual":
                            ...
                            state.status = "limit"
                            state.status_detail = (
                                f"Дневной лимит: {count}/{limit}. Сброс завтра в 00:00"
                            )
                        if not was_limited:
                            self._add_log(
                                state.short, state.color,
                                f"🛑 HH-счётчик: {count}/{limit}. Отклики остановлены",
                                "warning",
                            )
                    # Auto-recovery: если стоим в limit-stop, а реально count ниже лимита.
                    elif (state.hard_stopped or state.paused_reason == "limit") and count < max(0, limit - 5):
    ...
    def _process_llm_replies_inner(self, state: AccountState) -> None:
        ...
        for i, neg_id in enumerate(candidates[:15]):  # limit to 15 per cycle
            ...
            try:
                ...
                is_owner, owner_name = self._llm_profile_owner(state, vacancy_id)
                if not is_owner:
                    log_debug(
                        f"LLM [{state.short}] {neg_id}: chat belongs to profile "
                        f"{owner_name!r}, skip duplicate processing"
                    )
                    continue
                ...
                if thread.get("needs_reply") and not _employer_message_requires_reply(employer_msg):
                    log_debug(f"LLM [{state.short}] {neg_id}: informational/rejection message, no reply needed")
                    upsert_interview(
                        neg_id,
                        acc=state.short,
                ...
                if _text_buttons:
                    ...
                    if not CONFIG.llm_auto_send:
                        with state._llm_drafts_lock:
                            state._llm_drafts[key] = btn_text
                        upsert_interview(
                            neg_id,
                            acc=state.short,
                            acc_color=state.color,
                        ...
                        self._add_log(
                            state.short,
                            state.color,
                            f"🤖 Черновик ответа роботу [{employer_short}]: {btn_text}",
                            "info",
                            neg_id=neg_id,
                        )
                        continue

...
function _llmDetectProvider(key) {
  const k = (key || '').trim();
  if (!k) return null;
  if (k.startsWith('sk-or-'))   return {name:'OpenRouter', base_url:'https://openrouter.ai/api/v1', model:'openrouter/free'};
  ...
  try {
    ...
    try {
      if (typeof sendCmd === 'function') {
        ...
        sendCmd({type:'set_config', key:'llm_auto_send', value:false});
      }
...
function llmInterviewsRender() {
  ...
  if (rows.length === 0) {
    if (statusF || acc || search) {
      tbody.innerHTML = `<tr><td colspan="9" style="text-align:center;color:var(--dim);padding:12px">Нет записей по выбранному фильтру/поиску</td></tr>`;
  ...
  tbody.innerHTML = rows.map(r => {
    ...
    const ratingSlot = r.vacancy_id
      ...
      : '';
    const sendAction = r.status === 'draft' && r.neg_id
      ? `<button class="btn btn-primary" style="padding:3px 7px;font-size:10px" onclick="llmSendDraft('${esc(r.neg_id)}', this)">Отправить</button>`
      : '';
...
async function llmSendDraft(negId, button) {
  if (!confirm('Отправить этот черновик работодателю в HH?')) return;
  const oldText = button?.textContent || 'Отправить';
  if (button) { button.disabled = true; button.textContent = 'Отправка…'; }
  try {
    const res = await fetch(`/api/interviews/${encodeURIComponent(negId)}/send`, {method:'POST'});
    const data = await res.json();
    if (!data.ok) throw new Error(data.error || 'Не удалось отправить');
    showToast('✅ Сообщение отправлено в HH', 'success');
    await llmInterviewsLoad();
  } catch (e) {
...
function updateCard(card, acc) {
  ...
  card.querySelectorAll('.smart-filter-num').forEach(inp => {
    ...
    if (!inp._bound) {
      ...
      // Колесо мыши над сфокусированным input[type=number] незаметно меняло
      // дневной лимит на десятки пунктов во время обычной прокрутки страницы.
      // Блокируем изменение значения колёсиком; лимит меняется только вручную.
